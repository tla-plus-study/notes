## Week 1

First three lectures of [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Introduction to TLA+
- State Machines in TLA+
- Resources and Tools

## Week 2 (~ 11.29—11.30)
Continue with [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Die Hard (lecture & implementation attempt)
- Prepare for Transactional Commit
    - Goal is to understand the scope of next lecture.
    - Skim over script [Script](https://lamport.azurewebsites.net/video/video5-script.pdf)

## Week 3 (~ 12.06—12.07)
Continue with [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Transactional Commit (lecture & implementation attempt)
- Prepare for Two-Phase Commit
    - Goal is to understand the scope of next lecture.
    - Skim over script [Script](https://lamport.azurewebsites.net/video/video6-script.pdf)

## Week 4 (~ 12.13—12.14)
Continue with [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Two-Phase Commit (lecture & implementation attempt)

## Week 5 (~ 12.20—12.21)
Continue with [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Paxos Commit (lecture & implementation attempt)

## Week 6 (~ 12.27—12.28)
Christmas break.

## Week 7 (~ 01.03—01.04)
Free week: choose any topic from the syllabus or yourself.
- Deliverable: short write-up or notes on what you explored.

## Week 8 (~ 01.10—01.11)
- Raft (1 hour implementation attempt)

## Week 9 (~ 01.17—01.18)
Continue with [TLA+ video course](https://lamport.azurewebsites.net/video/videos.html).
- Implementation (part 1 & part 2)
